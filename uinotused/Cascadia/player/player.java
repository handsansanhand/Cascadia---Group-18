...
public static int foxScoring(ArrayList<Text> gg) {
    int score = 0;
    boolean hasFox;

    for (Text text : gg) {
        hasFox = false;
        if (text.getText().contains("FOX")) {
            double centerX = text.getX();
            double centerY = text.getY();
            for (Text adjacentText : gg) {
                double adjX = adjacentText.getX();
                double adjY = adjacentText.getY();
                if ((adjX == centerX - 60 && adjY == centerY) ||
                    (adjX == centerX + 60 && adjY == centerY) ||
                    (adjX == centerX && adjY == centerY - 60) ||
                    (adjX == centerX && adjY == centerY + 60)) {
                    if (!adjacentText.getText().contains("FOX")) {
                        score++;
                    } else {
                        hasFox = true;
                    }
...
public static int salmonScoring(ArrayList<Text> gg) {
    int score = 0;
    int runLength = 0;
    boolean inRun = false;
    String prevLabel = "";
    ...
    for (Text text : gg) {
        String currentLabel = text.getText();
        if (currentLabel.contains("SALMON")) {
            if (!inRun) {
                // start of a new run
                inRun = true;
                runLength = 1;
            } else if (!currentLabel.equals(prevLabel)) {
                // end of a run, add score
                score += getSalmonScore(runLength);
                runLength = 1;
            } else {
...
private static int getSalmonScore(int runLength) {
    if (runLength == 2) {
        return 2;
    } else if (runLength == 3) {
        return 4;
